...
for n=1:length(segs)
    for m=1:length(d)
        fileID = fopen('C:\4nec2\out\colinear.nec','w');

        fprintf(fileID,'CM Two colinear half-wavelength dipoles.\r\nCE\r\n');
        fprintf(fileID,'GW %d %d %.6f %.6f %.6f %.6f %.6f %.6f %.6f\r\n', 1, segs(n), 0, 0, d(m)/2, 0, 0, d(m)/2+l, r);
        fprintf(fileID,'GW %d %d %.6f %.6f %.6f %.6f %.6f %.6f %.6f\r\n', 2, segs(n), 0, 0, -d(m)/2-l, 0, 0, -d(m)/2, r);
        fprintf(fileID,'GE 0\r\n');
        ...
        system('(echo C:\4nec2\out\colinear.nec & echo C:\4nec2\out\colinear.out) | C:\4nec2\exe\nec2dxs1k5.exe');
        clc;

        fileID = fopen('C:\4nec2\out\colinear.out', 'r');
